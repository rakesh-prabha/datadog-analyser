...
// ===============================================
// Centralized, reus    template: `As a business operations analyst, analyze this log data focusing on customer and revenue impact:
...
- Orders with Known Values: {{actualOrdersWithValues}} of {{uniqueOrders}}`mplates for different analysis types
...
const OPERATIONAL_TEMPLATE = new PromptTemplate({
    ...
    template: String.raw`You are an expert operations engineer specialized in analyzing application logs and identifying service health issues. I have provided a summary of {{errorCode}} "Service Unavailable" errors found in a recent Datadog log export.
...
const BUSINESS_TEMPLATE = new PromptTemplate({
    ...
    template: String.raw`As a business operations analyst, analyze this log data focusing on customer and revenue impact:
...
const EXECUTIVE_TEMPLATE = new PromptTemplate({
    ...
    template: String.raw`As a senior business consultant, provide an executive summary of this system incident for C-level stakeholders:

INCIDENT OVERVIEW:
- System Impact: {{errorCount}} service failures detected
- Customer Impact: {{uniqueCustomers}} customers affected
- Revenue Impact: $` + '{{estimatedRevenue}}' + String.raw` at risk
...
const TECHNICAL_TEMPLATE = new PromptTemplate({
    ...
    template: String.raw`You are a senior software engineer performing root cause analysis on a production incident:
